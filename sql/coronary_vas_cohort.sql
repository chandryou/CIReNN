...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (2001510,4305852,4006788,43527998,44816356)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2001510,4305852,4006788)
  and c.invalid_reason is null
...
WHERE C.condition_start_date >= DATEFROMPARTS(2008, 12, 30)
AND V.visit_concept_id in (9201,9203)
